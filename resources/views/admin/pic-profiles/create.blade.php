...
@section('title', 'Create PIC Profile')

@section('content')
<div class="max-w-2xl mx-auto space-y-6">
    <!-- Header -->
    <div>
        <h1 class="text-3xl font-bold text-gray-900">Create PIC Profile</h1>
        <p class="mt-2 text-sm text-gray-600">Add a new PIC profile for auto-filling property details</p>
    </div>

    <!-- Form -->
    <div class="bg-white rounded-xl shadow-lg p-6">
        <form action="{{ route('admin.pic-profiles.store') }}" method="POST" class="space-y-6">
            @csrf

            <!-- Reference Number -->
            <div>
                <label for="ref_number" class="block text-sm font-bold text-gray-700 mb-2">Reference Number</label>
                <input type="text" name="ref_number" id="ref_number" value="{{ old('ref_number') }}" required 
