...
<div class="bg-white rounded-xl shadow-md border border-gray-200 p-6">
    <h1 class="text-2xl font-bold text-slate-900 mb-6">Edit Package</h1>
    ...
    <form action="{{ route('admin.service-packages.update', $servicePackage) }}" method="POST">
        ...
        <div class="space-y-6">
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $servicePackage->name) }}" required
            ...
            <div>
                ...
                <textarea name="short_description" id="short_description" rows="5"
                          class="w-full rounded-md border-gray-300 shadow-sm focus:border-[#96A480] focus:ring focus:ring-[#96A480] focus:ring-opacity-50">{{ old('short_description', $servicePackage->short_description) }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" step="0.01" name="normal_price" id="normal_price" value="{{ old('normal_price', $servicePackage->normal_price) }}" required
                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-[#96A480] focus:ring focus:ring-[#96A480] focus:ring-opacity-50">
                    @error('normal_price')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="discount_price" id="discount_price" value="{{ old('discount_price', $servicePackage->discount_price) }}"
                           class="w-full rounded-md border-gray-300 shadow-sm focus:border-[#96A480] focus:ring focus:ring-[#96A480] focus:ring-opacity-50">
            ...
            <div class="flex items-center">
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $servicePackage->is_active) ? 'checked' : '' }}
